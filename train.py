...
from deepseecolor.losses import (
    ...
    EdgeAwareBackscatterLoss,
    MultiscaleFeatureLoss,
    PhysicalPriorLoss,
    WaterTypeAdaptiveLoss,
    BackscatterAttenuationConsistencyLoss,
    ImprovedEdgeSmoothLoss
)
from deepseecolor.depth_losses import SmoothDepthLoss

from lpipsPyTorch import lpips
from metrics import readImages, read_image
from render_uw import estimate_atmospheric_light, render_set, detect_scene_from_training_images
...
def training(model_params, opt_params, pipe_params, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    if hasattr(model_params, 'scene_name') and model_params.scene_name:
        ...
        try:
            ...
            if 'curasao' in source_path:
                scene_name = 'Curasao'
            elif 'iui3-redsea' in source_path:
                scene_name = 'IUI3-RedSea'
            elif 'japanesegradens-redsea' in source_path:
                scene_name = 'JapaneseGradens-RedSea'
            elif 'panama' in source_path:
                scene_name = 'Panama'
    ...
    water_type_criterion = WaterTypeAdaptiveLoss().cuda()
    bs_at_consistency_criterion = BackscatterAttenuationConsistencyLoss().cuda()
    improved_edge_smooth_criterion = ImprovedEdgeSmoothLoss().cuda()
    ...
    water_type_loss = torch.Tensor([0.0]).squeeze().cuda()
    bs_at_consistency_loss = torch.Tensor([0.0]).squeeze().cuda()
    improved_edge_smooth_loss = torch.Tensor([0.0]).squeeze().cuda()
    ...
    while iteration < opt_params.iterations + 1:
        ...
        if iteration > opt_params.seathru_from_iter and opt_params.do_seathru:
            if opt_params.use_depth_weighted_l1:
                Ll1 = depth_weighted_l1_loss(underwater_image_batch, gt_image, depth_image.detach())
            elif opt_params.use_depth_weighted_l2:
                Ll1 = depth_weighted_l2_loss(underwater_image_batch, gt_image, depth_image.detach())
            else:
                Ll1 = l1_loss(underwater_image_batch, gt_image)
            l_ssim = ssim(underwater_image_batch, gt_image)
        else:
            if opt_params.use_depth_weighted_l1:
        ...
        if opt_params.add_recon_depth_l1:
            if iteration > opt_params.seathru_from_iter and opt_params.do_seathru:
                dl1 = depth_weighted_l1_loss(underwater_image_batch, gt_image, depth_image.detach())
            else:
            ...
            if opt_params.do_seathru and iteration > opt_params.seathru_from_iter:
                if opt_params.alpha_binf_uw or opt_params.alpha_binf_render or opt_params.alpha_bg_opacities:
                    ...
                    if opt_params.alpha_binf_uw:
                        alpha_bg_loss += alpha_bg_criterion(underwater_image_batch.squeeze().detach(), torch.sigmoid(bs_model.B_inf.detach()), image_alpha)
                    ...
                    if opt_params.turn_off_bg_loss:
                        ...
                        alpha_bg_loss = alpha_bg_criterion(underwater_image_batch.squeeze().detach(), torch.sigmoid(learned_bg.detach()), image_alpha)
        ...
        if opt_params.use_gw_loss:
            if opt_params.use_render_for_gw:
                ...
                J = direct_batch.detach() / attenuation_map_batch
                image_batch = J
            else:
                image_batch = torch.unsqueeze(image, dim=0)
        ...
        if iteration > opt_params.seathru_from_iter and opt_params.do_seathru:
            if opt_params.use_rgb_sv_loss:
                image_batch = torch.unsqueeze(image, dim=0)
                rgb_sv_loss = rgb_sv_criterion(image_batch.detach(), direct_batch)
        ...
        if opt_params.use_dcp_loss:
            gt_rgb_batch = torch.unsqueeze(gt_image, dim=0)
            if opt_params.do_seathru and iteration > opt_params.seathru_from_iter:
                # direct_grad_only_through_bsmodel_batch = underwater_image_batch.detach() - backscatter_batch.detach()
                direct_reversefromgt_through_bsmodel_batch = gt_rgb_batch.detach() - backscatter_batch.detach()
                depth_image_batch = torch.unsqueeze(depth_image, dim=0)
        ...
        if opt_params.do_seathru and iteration > opt_params.seathru_from_iter:
            if opt_params.use_dsc_at_loss:
                gt_rgb_batch = torch.unsqueeze(gt_image, dim=0)
                direct_reversefromgt_detached = (gt_rgb_batch - backscatter_batch).detach()
                if opt_params.disable_attenuation:
                    J_through_atmodel = torch.zeros_like(direct_reversefromgt_detached)
                else:
                    J_through_atmodel = direct_reversefromgt_detached / attenuation_map_batch
        ...
        if opt_params.use_improved_edge_smooth and opt_params.do_seathru and iteration > opt_params.seathru_from_iter:
            gt_rgb_batch = torch.unsqueeze(gt_image, dim=0)
            depth_image_batch = torch.unsqueeze(depth_image, dim=0)
            improved_edge_smooth_loss = improved_edge_smooth_criterion(underwater_image_batch, depth_image_batch, gt_rgb_batch)
            loss += opt_params.improved_edge_smooth_lambda * improved_edge_smooth_loss
        ...
        with torch.no_grad():
            ...
            if opt_params.do_seathru and iteration > opt_params.seathru_from_iter:
                training_report(
                    ...
                    edge_aware_bs_loss=edge_aware_bs_loss,
                    multiscale_feature_loss=multiscale_feature_loss,
                    physical_prior_loss=physical_prior_loss,
                    water_type_loss=water_type_loss,
                    bs_at_consistency_loss=bs_at_consistency_loss,
                    improved_edge_smooth_loss=improved_edge_smooth_loss
                )
            else:
                training_report(
                    tb_writer, iteration, Ll1, loss, l1_loss, iter_start.elapsed_time(iter_end), testing_iterations, scene, render, (pipe_params, background),
...
def training_report(tb_writer, iteration, Ll1, loss, l1_loss, elapsed, testing_iterations, scene : Scene, renderFunc, renderArgs,
                    ...
                    edge_aware_bs_loss=None,
                    multiscale_feature_loss=None,
                    physical_prior_loss=None,
                    water_type_loss=None,
                    bs_at_consistency_loss=None,
                    improved_edge_smooth_loss=None
):
    if tb_writer:
        ...
        if multiscale_feature_loss is not None:
            tb_writer.add_scalar('train_losses/multiscale_feature_loss', multiscale_feature_loss.item(), iteration)
        if physical_prior_loss is not None:
            tb_writer.add_scalar('train_losses/physical_prior_loss', physical_prior_loss.item(), iteration)
        if water_type_loss is not None:
            tb_writer.add_scalar('train_losses/water_type_loss', water_type_loss.item(), iteration)
        if bs_at_consistency_loss is not None:
            tb_writer.add_scalar('train_losses/bs_at_consistency_loss', bs_at_consistency_loss.item(), iteration)
        if improved_edge_smooth_loss is not None:
            tb_writer.add_scalar('train_losses/improved_edge_smooth_loss', improved_edge_smooth_loss.item(), iteration)
        ...
        if at_model is not None:
            if type(at_model) is not AttenuateNetV3 and hasattr(at_model, 'attenuation_coef'):
